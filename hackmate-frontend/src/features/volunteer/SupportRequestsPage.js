...
export default function SupportRequestsPage() {
    ...
    const resolve = useMutation({
        mutationFn: (id) => volunteerApi.resolveSupportMessage(id),
        onSuccess: () => {
    ...
    const priorityStyle = (priority) => {
        if (priority === 'urgent' || priority === 'high')
            return { border: 'border-red-500', bg: 'bg-red-50', badge: 'bg-red-100 text-red-700' };
        if (priority === 'medium')
            return { border: 'border-yellow-500', bg: 'bg-yellow-50', badge: 'bg-yellow-100 text-yellow-700' };
    ...
    const MessageCard = ({ m }) => {
        ...
        return (_jsx("div", { className: `p-4 border-l-4 rounded-xl ${isOpen ? `${style.border} ${style.bg}` : 'border-gray-300 bg-gray-50'}`, children: _jsxs("div", { className: "flex items-start justify-between gap-3", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [m.subject && (_jsx("p", { className: "font-semibold text-gray-900 text-sm mb-1", children: m.subject })), _jsxs("div", { className: "flex items-center gap-2 mb-2 flex-wrap", children: [_jsx("span", { className: `text-xs px-2 py-0.5 rounded-full font-medium ${style.badge}`, children: m.priority }), _jsx("span", { className: `text-xs px-2 py-0.5 rounded-full ${isOpen ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-600'}`, children: isOpen ? '⏳ Open' : '✅ Resolved' }), _jsxs("span", { className: "text-xs text-gray-500 capitalize", children: ["from ", m.from_role] })] }), _jsx("p", { className: "text-sm text-gray-800 leading-relaxed", children: m.message }), _jsxs("div", { className: "flex items-center gap-3 mt-2 text-xs text-gray-500 flex-wrap", children: [_jsxs("span", { className: "flex items-center gap-1", children: [_jsx(Clock, { className: "w-3 h-3" }), " ", timeAgo(m.created_at)] }), (m.floor_number || m.room_number) && (_jsxs("span", { children: ["\uD83D\uDCCD Floor ", m.floor_number, " \u2014 Room ", m.room_number] }))] })] }), isOpen && (_jsxs("button", { onClick: () => resolve.mutate(m.id), disabled: resolve.isPending, className: "shrink-0 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white text-xs px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1", children: [_jsx(CheckCircle, { className: "w-3 h-3" }), " Resolve"] }))] }) }));
    };
    return (_jsxs("div", { className: "space-y-5", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-2xl font-bold text-gray-900", children: "Support Requests" }), _jsx("p", { className: "text-gray-500 text-sm mt-0.5", children: "Support requests from teams in your assigned area" })] }), _jsx("div", { className: "grid grid-cols-2 gap-4", children: [
